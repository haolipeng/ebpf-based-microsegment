...
| **NAT 支持** | 100% | Conntrack 集成、NAT 检测 (ENABLE_NAT_SUPPORT) |
| **IP 分片处理** | 100% | 分片重组跟踪 (ENABLE_IP_FRAGMENT_HANDLING) |
| **gRPC 通信** | 100% | Agent-Server Protocol Buffers 通信 |
| **Web UI** | 100% | React + TypeScript + Vite 前端 |
...
| 功能类别 | 优先级 | 功能清单 |
|---------|--------|----------|
| **核心隔离** | P0 | 默认拒绝策略（default_policy_map 已创建，需配置接口）|
...
#### 1. NAT 支持和检测 ✅
**状态**: 已完成
**实现方式**: 通过 `ENABLE_NAT_SUPPORT` 编译标志控制
**实现文件**: `src/bpf/headers/nat_support.h`
**功能**:
...
#### 2. 分片数据包处理 ✅
**状态**: 已完成
**实现方式**: 通过 `ENABLE_IP_FRAGMENT_HANDLING` 编译标志控制
**实现文件**: `src/bpf/headers/fragment_handler.h`
**功能**:
- IP 分片重组跟踪
- 分片状态 Map（记录首个分片信息）
- 后续分片关联到首个分片策略
