# policy-management 规范 (MODIFIED)

## 目的
本规范扩展了现有的 policy-management 规范，以支持基于组的策略规则（group-based policy rules）和策略编译（policy compilation）。它添加了高级策略抽象的需求，其中规则在组之间定义（而非 IP 之间），并编译为数据平面的基于 IP 的规则。

## 变更类型
**MODIFIED** - 扩展现有的 `openspec/specs/policy-management/spec.md`
