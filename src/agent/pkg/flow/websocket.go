...
type Client struct {
	// WebSocket connection (exported for handler access)
	Conn *websocket.Conn

	// Buffered channel for outbound messages (exported for handler access)
	Send chan []byte

	// Client subscription filters (exported for handler access)
	Filters *FlowFilter

	// Hub reference (exported for handler access)
	Hub *Hub

	// Unique client ID (exported for handler access)
	ID string

	// Last activity timestamp (exported for handler access)
...
type FlowFilter struct {
	...
	// Event type filter (e.g., "NEW", "UPDATE", "CLOSED")
...
func NewHub() *Hub {
	return &Hub{
		...
		broadcast:         make(chan *Flow, 256), // Buffer for 256 messages
...
func (h *Hub) Broadcast(flow *Flow) {
	// Non-blocking send to avoid blocking the collector
	select {
...
func (h *Hub) matchesFilter(flow *Flow, filter *FlowFilter) bool {
	if filter == nil {
	...
	if filter.Protocol != nil && flow.Protocol != *filter.Protocol {
		return false
	}

	// Check source IP filter
	if filter.SourceIP != nil && flow.SourceIP != *filter.SourceIP {
		return false
	}

	// Check destination IP filter
	if filter.DestIP != nil && flow.DestIP != *filter.DestIP {
		return false
	}

	// Check policy action filter
	if filter.PolicyAction != nil && flow.PolicyAction != *filter.PolicyAction {
		return false
	}

	// Check state filter
	if filter.State != nil && flow.State != *filter.State {
		return false
	}

	// Check event type filter
	if filter.EventType != nil && flow.EventType != *filter.EventType {
		return false
	}
